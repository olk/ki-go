...
def _process_sgf(sgf_p, encoder, shape):
    ...
    game_state, first_move_done = _get_handicap(sgf)
        ...
        if color is not None:
            ...
            if first_move_done and point is not None:
                ...
                if not game_state.is_valid_move(move):
                    # exclude game with illegal move
                    return []
...
class SGFDataset(tfds.core.GeneratorBasedBuilder):
    """ SGF Go dataset """
    VERSION = tfds.core.Version('0.1.0')

    def __init__(self, encoder, train_frac, val_frac, sgf_files=0):
        self._encoder = encoder
        self._train_frac = train_frac
        self._val_frac = val_frac
        self._sgf_files = sgf_files
        VERSION = tfds.core.Version('0.1.%d' % self._sgf_files)
        super(SGFDataset, self).__init__()
