...
class Tree_node(Node):
    ...
    A Tree_node is a Node that also knows its position within an SGFGame.

    Do not instantiate directly; retrieve from an SGFGame or another Tree_node.
...
class _Unexpanded_root_tree_node(_Root_tree_node):
    """Variant of _Root_tree_node used with 'loaded' SGFGames."""
...
class SGFGame:
    ...
    def from_coarse_game_tree(cls, coarse_game, override_encoding=None):
        """Alternative constructor: create an SGFGame from the parser output.
    ...
    def from_string(cls, s, override_encoding=None):
        """Alternative constructor: read a single SGFGame from a string.
