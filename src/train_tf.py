...
from kigo.networks.keras import betago
...
def main():
    ...
    builder = SGFDatasetBuilder(encoder, train_frac, val_frac)
    ds_train = builder.as_dataset(split='train', as_supervised=True)
    ds_val = builder.as_dataset(split='validation', as_supervised=True)
    train_n = SGFDatasetBuilder.size(ds_train)
    val_n = SGFDatasetBuilder.size(ds_val)
